...
int evaluatePoint(char board[BOARD_WIDTH][BOARD_WIDTH], Position p) {
    ...
    for (i = p.x + min(min(p.y, BOARD_WIDTH - 1 - p.x), 5), j = p.y - min(min(p.y, BOARD_WIDTH - 1 - p.x), 5); i >= (unsigned int)max(0, p.x - 5) && i < 1000 && j < (unsigned int)min(BOARD_WIDTH, p.y + 6); i--, j++) {
        if (i != p.x) {
...
void updateScore(char board[BOARD_WIDTH][BOARD_WIDTH], Position p) {
    ...
    //斜杠
    for (i = p.x + min(p.y, BOARD_WIDTH - 1 - p.x), j = p.y - min(p.y, BOARD_WIDTH - 1 - p.x); i >= 0 && i< 1000 && j < BOARD_WIDTH; i--, j++) {
